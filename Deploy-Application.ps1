...
Try {
	...
	[string]$appVendor = 'HLP'
	[string]$appName = 'Chameleon Admin'
	[string]$appVersion = '46.0.0.23'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.1'
	[string]$appScriptDate = '2022.09.09'
	...
	$CloseApps = "Chameleon, Crystal Reports Runtime, cmd=Chameleon"
    
	##* Prerequisite Application install parameters. 
	# Prerequisite application 1 name.
	$PrereqApp1Name = "Crystal Reports Runtime"
	# Prerequisite application 1 install path.
	$PrereqApp1InstallPath = "$PSScriptRoot\Files\CRRuntime_32bit_13_0_22.msi"
	# Prerequisite application 1 install parameters.
	$PrereqApp1InstallParam = "REBOOT=ReallySuppress /QN"

	##* Application install parameters.
	# 64-bit application
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\Setup46g.exe"
	# Application  install parameters.
	$64bitAppInstallParam = '/VERYSILENT /CID="CONFIDENTIAL" /ZIP="CONFIDENTIAL" /S="CONFIDENTIAL" /M="CONFIDENTIAL" /COMPONENTS="R\A,T"'
	...
	$RemoveAppNamesMSI = @("Crystal Reports Runtime")
	$RemoveAppNamesEXE = @("Chameleon")

	##* Prerequisite Application uninstall parameters.
	# Prerequisite application 1 name.
	$PrereqApp2Name = "Crystal Reports 11"
	# Prerequisite application 1 uninstall path.
	$PrereqApp2UninstallPath = "C:\Windows\System32\MsiExec.exe"
	# Prerequisite application 1 uninstall parameters.
	$PrereqApp2UninstallParam = "/X{61D8C1B4-C6BD-40E3-B954-4D3B6E3ADE9C} /QN /quiet"

	##* Application uninstall parameters.
	# 64-bit application
	# Application uninstall path.
	$64bitAppUninstallPath = "$Env:Programfiles\Chameleon Software\Chameleon\unins000.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/VERYSILENT /QN"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If (Test-Path -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{61D8C1B4-C6BD-40E3-B954-4D3B6E3ADE9C}") {
			
			Try {
				Write-Log -Message "Previous versions of $PrereqApp2Name exist. Removing..."
				Execute-Process -FilePath "$PrereqApp2UninstallPath" -Arguments "$PrereqApp2UninstallParam"
				Write-Log -Message "Removing previous versions of $PrereqApp2Name."
			}
			# If removal failed, log results. Exit script. 
			Catch [System.Exception] {
				Write-Log -Message "Removing previous versions of $PrereqApp2Name failed with error: $_"
		...
		Else { 
			Write-Log -Message "Previous versions of $PrereqApp2Name do not exist."  
		}
		...
		Foreach ($RemoveAppNameEXE in $RemoveAppNamesEXE) {
			...
			If ($null -ne $RemoveAppNamesEXECheck) {
				...
				Try {
					...
					Remove-Folder -Path "$Env:Programfiles\Chameleon Software"
				}
				# If uninstall failed, log results. Exit script.
				Catch [System.Exception] {
